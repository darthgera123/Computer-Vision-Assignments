...
#	the image that should be processed by the algorithm.
	...
	def SetPreRegionSet(self, tf):
		self.PreRegionSet = tf
		self.RegionSet = tf
		self.Regionpos1 = wx.Point(int(coords1[0]),int(coords1[1]))
		self.Regionpos2 = wx.Point(int(coords1[2]),int(coords1[3]))

	def GetPreRegionSet(self):
		return self.PreRegionSet
...
#	of the application window,including several buttons.
	def __init__(self, parent, ID, doodle):
		...
		self.buttons.append(wx.Button(self, -1, "Set Predefined Region"))
		...
		self.Bind(wx.EVT_BUTTON, self.OnSetPreRegion, self.buttons[1])
		self.Bind(wx.EVT_BUTTON, self.OnSetRegion, self.buttons[2])
		self.Bind(wx.EVT_BUTTON, self.OnSetForeground, self.buttons[3])
		self.Bind(wx.EVT_BUTTON, self.OnSetBackground, self.buttons[4])
		self.Bind(wx.EVT_BUTTON, self.OnClear, self.buttons[5])
		self.Bind(wx.EVT_BUTTON, self.OnRun, self.buttons[6])
		self.Bind(wx.EVT_CHECKBOX, self.OnCheck, self.buttons[7])
	...
	def OnOpen(self, event):
		dlg = wx.FileDialog(self, "Open image file...", os.getcwd()+"/"+infile,
							style=wx.FD_OPEN,
	...
	def OnSetPreRegion(self, event):
		...
		if fn==[]:
			...
			self.doodle.SetLinesData([])
			self.doodle.ClearGroundData()
			self.doodle.SetPreRegionSet(True)
			self.doodle.SetDrawRegion(True)
	...
	def OnRun(self, event):
		...
		if fn==[]:
			...
			if algo_grabcut(filename=fn,
				...
				im = Image.open(os.getcwd()+'/'+outfile+'.png')
				im.show()
